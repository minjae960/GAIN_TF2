...
Data_Name = ['1_Basic_1_Seoul', '1_Basic_2_BR', '1_Basic_3_Ulsan', '2_Informed_1_Seoul', '2_Informed_2_BR', '2_Informed_3_Ulsan_raw.csv']
Missing_Col = ['ocec', 'ele', 'ion', 'ocec+ele', 'ocec+ion', 'ele+ion', 'all']
missing_rate = 0.2
hint_rate = 0.9
alpha = 100
epochs = 1
sq = 1000
Random_State = [777, 1004, 322, 224, 417]
...
for seed in Random_State:
    for missing_col in Missing_Col:
        for data_name in Data_Name:
            ...
            data = pd.read_csv('data/{}_raw.csv'.format(data_name)).drop(columns='date')
            ...
            def missing_sampler(rate, rows, cols, type):
                ...
                col_dic = {'ocec': ['OC', 'EC'],
                           ...
                           'ocec+ele': ['OC', 'EC', 'S', 'K', 'Ca', 'Ti', 'V', 'Cr', 'Mn', 'Fe', 'Ni', 'Cu', 'Zn', 'As', 'Se', 'Br',
                                        'Pb'],
                           'ocec+ion': ['OC', 'EC', 'SO42-', 'NO3-', 'Cl-', 'Na+', 'NH4+', 'K+', 'Mg2+', 'Ca2+'],
                           'ele+ion': ['S', 'K', 'Ca', 'Ti', 'V', 'Cr', 'Mn', 'Fe', 'Ni', 'Cu', 'Zn', 'As', 'Se', 'Br', 'Pb',
                                       'SO42-', 'NO3-', 'Cl-', 'Na+', 'NH4+', 'K+', 'Mg2+', 'Ca2+'],
                           'all': ['OC', 'EC', 'SO42-', 'NO3-', 'Cl-', 'Na+', 'NH4+', 'K+', 'Mg2+', 'Ca2+', 'S', 'K', 'Ca', 'Ti',
                                   'V', 'Cr', 'Mn', 'Fe', 'Ni', 'Cu', 'Zn', 'As', 'Se', 'Br', 'Pb']
                           }

                if type == 'data_m':
                    random_row = data.sample(int(len(data) * rate), random_state=seed).index.tolist()
                elif type == 'hint':
            ...
            def train_step(batch_x, batch_m):
                ...
                H_mb = batch_m * missing_sampler(1-hint_rate, sq, cols, 'hint')
            ...
            def train_lstm_gain(epochs): # inputting (1, sq, cols) size data with random order
                ...
                for epoch in range(epochs):
                    print('missing column is {} and random seed is {}'.format(missing_col, seed))
                    start = time.time()
                    i = 0
            ...
            # 3-5) save data
            def save_csv():
                GAIN = pd.DataFrame(GAIN_imputed)
                KNN = pd.DataFrame(KNN_imputed)
